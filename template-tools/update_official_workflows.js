// Script to update existing templates with official workflow definitions
...
const logFile = path.join(logsDir, 'template_update.log');
...
function initLogFile() {
  ...
  const logHeader = `=== n8n Official Template Update Log - ${timestamp} ===\n\n`;
...
function createDirectories() {
  ...
  log('Ensured directory structure exists');
}
...
async function updateTemplate(template) {
  try {
    ...
    if (!templateExists) {
      ...
      log(`Created new directory for template ${template.id}`);
    } else {
      log(`Template directory for ${template.id} already exists`);
    }
    ...
    try {
      ...
      if (templateExists) {
        log(`Updated official workflow for template ${template.id}: ${metadata.name}`);
      } else {
        log(`Added new official workflow for template ${template.id}: ${metadata.name}`);
      }
...
async function updateTemplates() {
  try {
    ...
    for (const template of firstPageData.workflows) {
      ...
      const metadata = await updateTemplate(template);
      if (metadata) {
    ...
    log(`Processed ${firstPageData.workflows.length} templates from page 0 (Success: ${successCount}, Errors: ${errorCount}, Updated: ${updatedCount}, New: ${newCount})`);
    ...
    for (let page = 1; page < totalPages; page++) {
      try {
        ...
        log(`Processed ${pageData.workflows.length} templates from page ${page}/${totalPages-1} (Success: ${pageSuccessCount}, Errors: ${pageErrorCount}, Duplicates: ${pageDuplicateCount}, Updated: ${pageUpdatedCount}, New: ${pageNewCount})`);
      } catch (error) {
    ...
    log(`- Updated templates: ${updatedCount}`);
    log(`- New templates: ${newCount}`);
    ...
    log(`- Template directories: ${templateDirs}`);
    ...
    log(`Fatal error updating templates: ${error.message}`, true);
  }
}
